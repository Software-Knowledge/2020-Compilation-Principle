...
1. 如果文法G 的SLR(1) 分析表是无冲突的, 则G 是SLR(1) 文法。
...
1. 即使考虑$= \in Follow(A)$, 对该文法来说仍然不够
2. 因为, 这仅仅说明在某个句型中, a 可以跟在A 后面
3. 该文法没有以R = · · · 开头的最右句型

![](img/lec3/64.png)

3. 希望LR 语法分析器的每个状态能**尽可能精确**地
4. 指明哪些输入符号可以跟在句柄$A \rightarrow \alpha$的后面
5. 在LR(0) 自动机中, 某个项集Ij 中包含$[A \rightarrow α·]$
6. 则在之前的某个项集Ii 中包含$[B \rightarrow \beta · A\gamma]$
7. 这表明只有$a \in First(\gamma)$时, 才可以进行$A \rightarrow \alpha$归约
8. 但是, 对$I_i$求闭包时, 仅得到$[A \rightarrow ·\alpha]$, 丢失了$First(\gamma)$信息

## 5.13. Definition (LR(1) 项(Item))
1. $[A \rightarrow \alpha·\beta, a] (a \in T \cup {\$})$，此处, a 是向前看符号, 数量为1.
2. 此处, a 是向前看符号, 数量为1.
3. 思想: $\alpha$在栈顶, 且输入中开头的是可以从$\beta\alpha$推导出的符号串
