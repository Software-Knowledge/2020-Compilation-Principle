...
### 5.7.1. 状态刻画了“当前观察到的针对所有产生式的右部的前缀”

#### 5.7.1.1. Definition (LR(0) 项(Item))
...
### 5.7.2. 状态刻画了“当前观察到的针对所有产生式的右部的前缀”

#### 5.7.2.1. Definition (项集)
1. 项集就是若干项构成的集合。
2. 因此, 句柄识别自动机的一个状态可以表示为一个项集

#### 5.7.2.2. Definition (项集族)
1. 项集族就是若干项集构成的集合。
2. 因此, 句柄识别自动机的状态集可以表示为一个项集族

#### 5.7.2.3. Definition (增广文法(Augmented Grammar))
...
### 5.7.5. LR(0) 分析表每一行(状态) 所选用的归约产生式是相同的
![](img/lec3/52.png)
