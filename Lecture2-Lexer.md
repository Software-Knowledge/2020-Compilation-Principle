...
1. 字符串x对应于有向图中的路径
2. 求有向图中所有(从初始状态到接受状态的) 路径
3. **但是, 如果有向图中含有环, 则存在无穷多条路径**
4. **不要怕, 我们有Kleene闭包**
5. 假设有向图中节点编号为0(**初始状态**)到n − 1
6. R^k_{ij}:从节点i到节点j、且**中间节点**编号**不大于k**的所有路径

![](img/lec2/54.png)

$$
R_{ij}^k = R_{ik}^{k-1}(R_{kk}^{k-1}^*R_{ij}^{k-1})
$$

1. Q : 如何初始化?
2. $R_{ij}^{-1}$：从节点i 到节点j、且不经过中间节点的所有路径

![](img/lec2/55.png)

1. 关于$\emptyset$ (注意: 它不是正则表达式) 的规定
...
3. 合并这三个NFA,构造$a|abb|a^∗b^+$对应的NFA

![](img/lec2/63.png)

4. 使用子集构造法将NFA转换为DF(并消除死状态$\emptyset$)

![](img/lec2/64.png)

5. 注意:要保留各个NFA的**接受状态**信息,并采用**最前优先匹配**原则
6. 模拟运行该DFA,**直到无法继续为止**(输入结束或状态无转移);假设此时状态为s
7. 若s为接受状态,则识别成功;
8. 否则,**回溯**(包括状态与输入流)至最近一次经过的接受状态,识别成功;若没有经过任何接受状态,则**报错**(忽略第一个字符)
9. 无论成功还是失败,都从**初始状态**开始继续识别下一个词法单元

![](img/lec2/65.png)

1. x=a输入结束;接受;识别出a
2. x=abba状态无转移;接受;识别出abb
3. x=aaaa输入结束;回溯成功;识别出a
4. x=cabb状态无转移;回溯失败;报错c
