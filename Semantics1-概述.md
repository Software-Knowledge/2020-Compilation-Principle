...
|                     |                      |
| ------------------- | -------------------- |
| ![](img/lec4/9.png) | ![](img/lec4/10.png) |
...
## 1.18. Definition (语法制导的翻译方案(Syntax-Directed Translation Scheme;SDT))
...
### 1.19.1. 按照从左到右的深度优先顺序遍历语法分析树
![](img/lec4/28.png)
![](img/lec4/29.png)

### 1.19.2. 嵌入语义动作虚拟节点的语法分析树
![](img/lec4/30.png)
![](img/lec4/31.png)
...
4. A→X1···Xi···Xn
5. 原则:从左到右处理各个Xi符号对每个Xi,先计算继承属性,后计算综合属性

## 1.21. 递归下降子过程A→X1···Xi···Xn
1. 在调用Xi子过程之前,计算Xi的继承属性
2. 以Xi的继承属性为参数调用Xi子过程
3. 在Xi子过程返回之前,计算Xi的综合属性
4. 在Xi子过程中返回Xi的综合属性

![](img/lec4/40.png)

## 1.22. 继承属性R.i用于计算并传递中间结果
![](img/lec4/41.png)
...
2. L属性定义转换为SDT
3. A→X1···Xi···Xn
4. 计算Xi继承属性的动作放在产生式体Xi的左边
5. 计算产生式头部A综合属性的动作放在产生式体的最右边
